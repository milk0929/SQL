...
Select stock_name, running_total as capital_gain_loss 
from (
select 
stock_name,
sum(price) over(partition by stock_name order by operation_day) as running_total,
row_number() over(partition by stock_name order by operation_day desc) as r_n 
from full_t) t 
where r_n = 1 