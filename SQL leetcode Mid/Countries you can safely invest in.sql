...
--- Unsucessful try
with id_c as (
Select id, sunstring(phone_number,1,3) as c_code 
from Person
),
...
with id_c_2(
    ...
    from id_c 
    left join Country 
    on id_c.c_code = Country.country_code
)

Select *, 
i.name as callee_country,
j.name as caller_country
from 
Calles c
LEFT JOIN id_c_2 i on i.id = c.callee_id
left join id_c_2 j on j.id = c.caller_id 
...
WITH cte AS (
    ...
    FROM 
        person p
    JOIN 
        country c ON SUBSTRING(p.phone_number, 1, 3) = c.country_code
),
cte2 AS (
    SELECT 
        cte.country,
        calls.duration,
        AVG(calls.duration) OVER (PARTITION BY cte.country) AS country_avg_duration,
        AVG(calls.duration) OVER () AS global_avg_duration
    FROM 
        cte
    JOIN 
        calls ON cte.id = calls.caller_id OR cte.id = calls.callee_id
)
...
JOIN calls ca 
on p.id = ca.callee_id or p.id = ca.caller_id 
group by c.name
